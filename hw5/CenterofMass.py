...
#---Output: returns the 3D coords of the COM(position OR velocity)
    ...
    def COMdefine(self, a,b,c, m):
        ...
        totmass = np.sum(m)
        #---define COM of x,y,z
        ACOM = np.sum(a*m)/(totmass)
        BCOM = np.sum(b*m)/(totmass)
        CCOM = np.sum(c*m)/(totmass)
...
#---Output: returns an array with the x,y,z coords of the COM position
    ...
    def COM_V(self, delta):
        ...
        #---change to COM frame
        VXNEW = (self.x[:]*u.kpc - COMP[0])
        VYNEW = (self.y[:]*u.kpc - COMP[1])
        VZNEW = (self.z[:]*u.kpc - COMP[2])
