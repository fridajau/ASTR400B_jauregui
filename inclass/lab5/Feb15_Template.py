...
yD = COMD.y - float(COMP[0]/u.kpc)
zD = COMD.z - float(COMP[0]/u.kpc)

vxD = COMD.vx - float(COMV[0]/(u.km/u.s))
vyD = COMD.vy - float(COMV[0]/(u.km/u.s))
vzD = COMD.vz - float(COMV[0]/(u.km/u.s))
...
#### PLACE FUNCTION HERE ####
plt.hist2d(xD, yD, bins=1000, norm=LogNorm())
plt.colorbar()
...
#### PLACE FUNCTION HERE ####
plt.hist2d(yD, zD, bins=1000, norm=LogNorm())
plt.colorbar()
...
plt.hist2d(xD, vyD, bins=1000, norm=LogNorm())
plt.colorbar()


# Add axis labels
plt.xlabel('x (kpc)', fontsize=22)
plt.ylabel('Velocity vy (km/s)', fontsize=22)
...
plt.plot(-1*R,-1*VCirc, color='black')
